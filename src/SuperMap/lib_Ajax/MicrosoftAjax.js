...
Function._validateParameterCount = function (e, a) {
    var c = a.length, d = 0;
    for (var b = 0; b < a.length; b++)if (a[b].parameterArray)c = Number.MAX_VALUE; else if (!a[b].optional)d++;
...
Number._parse = function (g, f) {
    ...
    var d = f.numberFormat, b = d.NumberDecimalSeparator, c = d.NumberGroupSeparator, e = new RegExp("^[+-]?[\\d\\" + c + "]*\\" + b + "?\\d*([eE][+-]?\\d+)?$");
...
Number.prototype._toFormattedString = function (d, j) {
    ...
    function i(p, k, j, l, o) {
        ...
        if (k > 0) {
            var f = a.length - k;
            if (f > 0)a = a.slice(0, k); else if (f < 0)for (var n = 0; n < Math.abs(f); n++)a += "0";
            a = o + a
        } else a = "";
        a += m;
        var b = c.length - 1, d = "";
...
String._toFormattedString = function (l, j) {
    ...
    for (var a = 0; true;) {
        ...
        if (d < 0)break;
        var h = e.substring(a, d), g = h.indexOf(":"), k = parseInt(g < 0 ? h : h.substring(0, g)) + 1, i = g < 0 ? "" : h.substring(g + 1), b = j[k];
...
var $create = Sys.Component.create = function (h, f, d, c, g) {
    ...
    if (i)if (c)b._addComponentToSecondPass(a, c); else a.endUpdate(); else {
...
Sys.UI.DomEvent = function (c) {
    ...
    if (a.type === "keypress")this.charCode = a.charCode || a.keyCode; else if (a.keyCode && a.keyCode === 46)this.keyCode = 127; else this.keyCode = a.keyCode;
...
Sys.UI.DomEvent.prototype = {preventDefault:function () {
    if (this.rawEvent.preventDefault)this.rawEvent.preventDefault(); else if (window.event)window.event.returnValue = false
}, stopPropagation:function () {
    if (this.rawEvent.stopPropagation)this.rawEvent.stopPropagation(); else if (window.event)window.event.cancelBubble = true
}};
...
var $addHandler = Sys.UI.DomEvent.addHandler = function (a, d, e) {
    ...
    var d = null, c = a._events[e], d = null;
...
Sys._ScriptLoader.prototype = {dispose:function () {
    ...
    if (b && this._currentTask._notified)if (this._currentTask._notified > 1)this._raiseError(true); else {
...
Sys.Net.WebServiceProxy.prototype = {set_timeout:function (a) {
    this._timeout = a
}, get_timeout:function () {
    return this._timeout
...
Sys.Net.WebServiceProxy.invoke = function (k, a, j, d, i, c, f, h) {
    ...
    function l(d) {
        if (d.get_responseAvailable()) {
            var e = d.get_statusCode(), b = null;
            try {
                var j = d.getResponseHeader("Content-Type");
                if (j.startsWith("application/json"))b = d.get_object(); else if (j.startsWith("text/xml"))b = d.get_xml(); else b = d.get_responseData()
            } catch (m) {
            }
            var k = d.getResponseHeader("jsonerror"), g = k === "true";
            ...
            var h;
            if (d.get_timedOut())h = String.format(Sys.Res.webServiceTimedOut, a); else h = String.format(Sys.Res.webServiceFailedNoMsg, a);
...
Sys.Services._ProfileService.prototype = {_defaultFailedCallback:null, _defaultLoadCompletedCallback:null, _defaultSaveCompletedCallback:null, _path:"", _timeout:0, get_defaultFailedCallback:function () {
    ...
    var c = {}, a;
    if (!b)a = "GetAllPropertiesForCurrentUser"; else {
        a = "GetPropertiesForCurrentUser";
        c = {properties:this._clonePropertyNames(b)}
    }
    ...
    this._invoke(this._get_path(), "SetPropertiesForCurrentUser", false, {values:b}, Function.createDelegate(this, this._onSaveComplete), Function.createDelegate(this, this._onSaveFailed), [a, c, e])
    ...
    var c = f[0], e = c ? c : this._defaultLoadCompletedCallback;
    if (e)e(a.length, f[2], "Sys.Services.ProfileService.load")
}, _onLoadFailed:function (d, c) {
    var a = c[1], b = a ? a : this._defaultFailedCallback;
    ...
    var a = c[0], e = c[2], b = a ? a : this._defaultSaveCompletedCallback;
    if (b)b(d, e, "Sys.Services.ProfileService.save")
}, _onSaveFailed:function (e, c) {
    var a = c[1], d = c[2], b = a ? a : this._defaultFailedCallback;
...
Sys.Services._AuthenticationService.prototype = {_defaultFailedCallback:null, _defaultLoginCompletedCallback:null, _defaultLogoutCompletedCallback:null, _path:"", _timeout:0, _authenticated:false, get_defaultFailedCallback:function () {
    return this._defaultFailedCallback
}, set_defaultFailedCallback:function (a) {
    this._defaultFailedCallback = a
    ...
    this._invoke(this._get_path(), "Login", false, {userName:c, password:b, createPersistentCookie:a}, Function.createDelegate(this, this._onLoginComplete), Function.createDelegate(this, this._onLoginFailed), [c, b, a, f, d, e, g])
    ...
    var b = c[3], d = c[4], e = c[6], a = d ? d : this._defaultLoginCompletedCallback;
    if (f) {
    ...
    var a = c[5], b = a ? a : this._defaultFailedCallback;
    ...
    var c = a[0], b = a[1], e = a[3], d = b ? b : this._defaultLogoutCompletedCallback;
    this._authenticated = false;
    if (d)d(null, e, "Sys.Services.AuthenticationService.logout");
    if (!c)window.location.reload(); else window.location.href = c
}, _onLogoutFailed:function (d, c) {
    var a = c[2], b = a ? a : this._defaultFailedCallback;
...
Sys.Serialization.JavaScriptSerializer._serializeWithBuilder = function (b, a, h) {
    ...
    switch (typeof b) {
        case "object":
            ...
            break;
        case "number":
            if (isFinite(b))a.append(String(b)); else throw Error.invalidOperation(Sys.Res.cannotSerializeNonFiniteNumbers);
            break;
        case "string":
            a.append('"');
...
switch (Sys.Browser.agent) {
    case Sys.Browser.InternetExplorer:
        Sys.UI.DomElement.getLocation = function Sys$UI$DomElement$getLocation(a) {
            ...
            var e = a.ownerDocument.parentWindow, g = e.screenLeft - top.screenLeft - top.document.documentElement.scrollLeft + 2, h = e.screenTop - top.screenTop - top.document.documentElement.scrollTop + 2, c = e.frameElement || null;
            if (c) {
...
Sys.UI.Control.prototype = {_parent:null, _visibilityMode:Sys.UI.VisibilityMode.hide, get_element:function () {
    ...
    if (this._parent)return this._parent; else {
    ...
    if (this._visibilityMode !== a) {
        ...
        if (this.get_visible() === false)if (this._visibilityMode === Sys.UI.VisibilityMode.hide)this._element.style.display = this._oldDisplayMode; else this._element.style.display = "none"
    }
    this._visibilityMode = a
    ...
    if (a != this.get_visible()) {
        ...
        if (a || this._visibilityMode === Sys.UI.VisibilityMode.hide)this._element.style.display = this._oldDisplayMode; else this._element.style.display = "none"
    }
